...
export const Sidebar = () => {
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 flex flex-col h-screen select-none shrink-0 sticky top-0">
      ...
      <div className="p-3 border-t border-slate-200 bg-slate-50/50 m-3 rounded-xl border">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-full bg-slate-200 border border-slate-300 overflow-hidden flex items-center justify-center text-slate-600 font-bold shrink-0">
            {user?.username ? user.username.slice(0, 2).toUpperCase() : <User className="w-5 h-5" />}
          </div>
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-slate-500 truncate">
              {user?.role === 'ADMIN' ? 'System Admin' : 'Border Officer'}
            </p>
